...
export const MODULE_CATALOG: Record<string, ModuleCatalogEntry> = {
  ...
  collecte_valeur: {
    ...
    permissions: [
      ...
      // Dynamic permissions per workflow validation node are added at runtime:
      // {node_id}_read  → "{Node name} lecture"
      // {node_id}_validate → "{Node name} validation"
    ],
  },
  assurance: {
    slug: 'assurance',
    ...
    hasBo: true, hasWorkflows: false, hasRoles: true,
    permissions: [],
  },
  profils: {
    slug: 'profils',
...
export function getModuleCatalog(slug: string): ModuleCatalogEntry | undefined {
